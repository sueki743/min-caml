  ...
  try
    ...
    let live = (* 生きているレジスタ *)
	  ...
	  if is_reg y then S.add y live else
          ...
          with Not_found -> live)(*これは外部変数の場合？*)
        S.empty
        free in
    let r = (* そうでないレジスタを探す *)
      List.find
...
let rec g dest cont regenv = function (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  ...
  | Let((x, t) as xt, exp, e) ->
     assert (not (M.mem x regenv));(*a変換後なので、同じ変数が複数回代入されることはないから？*)
     ...
     (*xtをdestにして計算*)
      ...
      | Alloc(r) ->
	 let (e2', regenv2) = g dest cont (add x r regenv1) e in(*再帰*)
...
and g'_call dest cont regenv exp constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (List.fold_left
     ...
     (Ans(constr
	    ...
	    (List.map (fun z -> find z Type.Float regenv) zs)))(*この時点でys,zsのレジスタ割り当ては終わっているので、置き換える*)
     (fv cont),
   M.empty)
...
let h { name = Id.L(x); args = ys; fargs = zs; body = e; ret = t } = (* 関数のレジスタ割り当て (caml2html: regalloc_h) *)
  ...
  let a =
    ...
    | Type.Float -> fregs.(0)
    | _ -> regs.(0) in
  let (e', regenv') = g (a, t) (Ans(Add(a,C(0)))) regenv e in
  { name = Id.L(x); args = arg_regs; fargs = farg_regs; body = e'; ret = t }

let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  Format.eprintf "register allocation: may take some time (up to a few minutes, depending on the size of functions)@.";
  let fundefs' = List.map h fundefs in
  let e', regenv' = g (Id.gentmp Type.Unit, Type.Unit) (Ans(Nop)) M.empty e in(*全体はユニット型だから、dest*)
  Prog(data, fundefs', e')
